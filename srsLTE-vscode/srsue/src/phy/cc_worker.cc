...
cc_worker::cc_worker(uint32_t cc_idx_, uint32_t max_prb, srsue::phy_common* phy_, srslte::log* log_h_)
{
  ...
  printf("SRSLTE_SF_LEN_PRB(max_prb) = %d\n", SRSLTE_SF_LEN_PRB(max_prb));
  printf("signal_buffer_max_samples = %d\n", signal_buffer_max_samples);

  // /* filesink init */
  // srslte_filesink_init(&sink1, (char *)"/home/hadoop/Bjz_workplace/srsLTE-vscode/data/IQ_data1.csv", SRSLTE_COMPLEX_FLOAT_BIN);
  // srslte_filesink_init(&sink2, (char *)"/home/hadoop/Bjz_workplace/srsLTE-vscode/data/ce_data1.csv", SRSLTE_COMPLEX_FLOAT_BIN);
  // srslte_filesink_init(&sink3, (char *)"/home/hadoop/Bjz_workplace/srsLTE-vscode/data/channel_res_data1.csv", SRSLTE_FLOAT_BIN);

  /* FILE INIT */
  // F1 = fopen("/home/hadoop/Bjz_workplace/srsLTE-vscode/data/IQ_data1.bin","w+");
  // F2 = fopen("/home/hadoop/Bjz_workplace/srsLTE-vscode/data/ce_data2.bin","w+");
  // FILE *F2=fopen("/home/hadoop/Bjz_workplace/srsLTE-vscode/data/ce_data1.bin","w+");
  // FILE *F6=fopen("/home/hadoop/Bjz_workplace/srsLTE-vscode/data/sf_data.txt","w+");
...
cc_worker::~cc_worker()
{
  ...
  /* filesink free */
  // srslte_filesink_free(&sink1);
  // srslte_filesink_free(&sink2);
  // srslte_filesink_free(&sink3);
...
bool cc_worker::work_dl_regular()
{
  ...
  // // 以下是我加的内容
  // uint32_t llen = get_buffer_len();
  // printf("get_buffer_len = %d\n", llen);
  // for(uint32_t i = 0; i < phy->args->nof_rx_ant; i++){
  //   for (uint32_t j = 0; j < llen; j++) {
  //     printf("float complex =%.8f+%.8fi\n",crealf(signal_buffer_rx[i][j]),cimagf(signal_buffer_rx[i][j]));
  //   }
  // }

  // 将IQ数据写入文件
  // // FILE *f1;
  // // f1 = fopen("/home/hadoop/Bjz_workplace/srsLTE-vscode/data/IQ_data1.csv","a");
  // printf("正在写入文件IQ_data1.csv...\n");
  // uint32_t llen = get_buffer_len();
  // for(uint32_t i = 0; i < phy->args->nof_rx_ant; i++){
  //   for (uint32_t j = 0; j < llen; j++) {
  //     //printf("float complex =%.8f+%.8fi\n",crealf(signal_buffer_rx[i][j]),cimagf(signal_buffer_rx[i][j]));
  ...
  //   }
  // }
  // printf("文件IQ_data1.csv完成本轮写入.\n");
  // // fclose(f1);

  // // 将IQ数据写入文件
  // printf("正在写入文件IQ_data1.csv...\n");
  // srslte_filesink_write_multi(&sink1, (void**)signal_buffer_rx, get_buffer_len(), phy->args->nof_rx_ant);
  // printf("文件IQ_data1.csv完成本轮写入.\n");
  ...
  // uint32_t llen = get_buffer_len();
  // for(uint32_t i = 0; i < phy->args->nof_rx_ant; i++){
  //   fwrite(signal_buffer_rx[i], 8, llen/16, F1);
  // }

  // if(ue_dl.fft[0].nof_re == 600 && ue_dl.fft[0].nof_symbols == 7)
  // {
  //   for (int i = 0; i < 1; i++)
  //   {
  //     /* code */
  //     fwrite(ue_dl.sf_symbols, 8, 8400, F6);
  //     // for (int j = 0; j < 8400; j=j+4)
  //     // {
  //     //   /* code */
  //     //   fwrite(&(ue_dl.sf_symbols[i][j]), 8, 1, F6);
  //     // }
