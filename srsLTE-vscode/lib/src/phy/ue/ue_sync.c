...
static int receive_samples(srslte_ue_sync_t* q, cf_t* input_buffer[SRSLTE_MAX_CHANNELS], const uint32_t max_num_samples)
{
  ...
  // printf("q->next_rf_sample_offset = %d\n", q->next_rf_sample_offset);
  ...
  // printf("max_num_samples = %d\n", max_num_samples);

  // printf("q->frame_len - q->next_rf_sample_offset = %d\n", q->frame_len - q->next_rf_sample_offset); // = 11520
  ...
  //   F4=fopen("/home/hadoop/Bjz_workplace/srsLTE-vscode/data/IQ_data1.txt","w+");
  // }

  // for (int i = 0; i < 1; i++){
  //    fwrite(&input_buffer[i][q->next_rf_sample_offset],8,q->frame_len - q->next_rf_sample_offset,F4);
  // }

  // for (int i = 0; i < 1; i++)
  // {
  //   for (int j = q->next_rf_sample_offset; j < q->frame_len - q->next_rf_sample_offset; j++)
  //   {
  //     printf("q->next_rf_sample_offset=%d, q->frame_len=%d",q->next_rf_sample_offset,q->frame_len);
  //     printf("%f+%fi\n",crealf(input_buffer[i][j]), cimagf(input_buffer[i][j]));
  //   }
  // }
                            ...
                            const uint32_t    max_num_samples)
{
  ...
  if (q != NULL && input_buffer != NULL) {
    ...
    if (q->file_mode) {
      ...
      printf("Reading %d samples. sf_idx = %d\n", q->sf_len, q->sf_idx);
      ret = 1;
    } else {
      ...
      // printf("q->frame_len = %d, q->fft_size = %d, q->sf_len = %d\n",q->frame_len, q->fft_size, q->sf_len);
      ...
      switch (q->state) {
        ...
        case SF_TRACK:
          ...
          // printf("q->sf_idx = %d  ",q->sf_idx);
          // printf("q->frame_number = %d\n",q->frame_number);
          q->sf_idx = (q->sf_idx + q->nof_recv_sf) % 10;
          // printf("q->nof_recv_sf = %d\n",q->nof_recv_sf); // =1
          if (q->sf_idx == 0) {
          ...
          if (q->mode == SYNC_MODE_PSS) {
            // printf("get in SYNC_MODE_PSS\n");
            srslte_ue_sync_run_track_pss_mode(q, input_buffer);
          } else {
            // printf("get in SYNC_MODE_GNSS\n");
