...
static int estimate_pdcch_pcfich(srslte_ue_dl_t* q, srslte_dl_sf_cfg_t* sf, srslte_ue_dl_cfg_t* cfg)//, FILE *F2, FILE *F3)
{
  if (q) {
    ...
    srslte_chest_dl_estimate_cfg(&q->chest, sf, &cfg->chest_cfg, q->sf_symbols, &q->chest_res);//, F2, F3);

    // 将信道数据（即，CSI）写入文件
    // FILE *f2;
    // f2 = fopen("/home/hadoop/Bjz_workplace/srsLTE-vscode/data/ce_data1.csv","a");
    // printf("正在写入文件ce_data1.csv...\n");
    // fprintf(f2, "[\n");
    // printf("[\n");
    ...
    //     // fprintf(f2, "    [\n");
    //     // printf("    [\n");
    //     for(uint32_t k=0; k<q->chest_res.nof_re; /*k<1;*/ ++k){
    //       // fprintf(f2, "%.10f+%.10fi,\n",crealf(q->chest_res.ce[i][j][k]), cimagf(q->chest_res.ce[i][j][k]));
    //       // printf("%.10f+%.10fi,\n",crealf(q->chest_res.ce[i][j][k]), cimagf(q->chest_res.ce[i][j][k]));
    //       // 打算调用filesink试试
    //     }
    //     // fprintf(f2, "    ],\n");
...
int srslte_ue_dl_decode_fft_estimate(srslte_ue_dl_t* q, srslte_dl_sf_cfg_t* sf, srslte_ue_dl_cfg_t* cfg)//, FILE *F2, FILE *F3)
{
  if (q) {
    ...
    return estimate_pdcch_pcfich(q, sf, cfg);//, F2, F3);
  } else {
    return SRSLTE_ERROR_INVALID_INPUTS;
  }
}
