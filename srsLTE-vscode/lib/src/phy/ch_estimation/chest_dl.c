...
int srslte_chest_dl_res_init(srslte_chest_dl_res_t* q, uint32_t max_prb)
{
  ...
  /* file open */
  // q->F2 = fopen("/home/hadoop/Bjz_workplace/srsLTE-vscode/data/ce_data1.bin","w+");
  // q->F3 = fopen("/home/hadoop/Bjz_workplace/srsLTE-vscode/data/channel_res_data1.bin","w+");
                               ...
                               uint32_t               port_id)
{
  ...
  if (sf->sf_type == SRSLTE_SF_NORM && (cfg->estimator_alg == SRSLTE_ESTIMATOR_ALG_AVERAGE || nsymbols < 3)) {
    // printf("just copy the estimates in the time domain\n"); // yes
    // If we average per subframe, just copy the estimates in the time domain
    // printf("2 * SRSLTE_CP_NSYMB(q->cell.cp)=%d",2 * SRSLTE_CP_NSYMB(q->cell.cp)); // =14
                                        ...
                                        uint32_t               rxant_id)
{
  ...
  if (q->wiener_dl && ch_mode == SRSLTE_SF_NORM && cfg->estimator_alg == SRSLTE_ESTIMATOR_ALG_WIENER) { // won't get in
                             ...
                             srslte_chest_dl_res_t* res)
{
  ...
  return srslte_chest_dl_estimate_cfg(q, sf, &cfg, input, res);//, NULL, NULL);
}
                                 ...
                                 //FILE *F3)
{
  ...
  // printf("q->nof_rx_antennas = %d\n",q->nof_rx_antennas); // =1
  // printf("q->cell.nof_ports = %d\n",q->cell.nof_ports); // =1
  ...
  fill_res(q, res);
  // printf("res->noise_estimate = %f \n", res->noise_estimate);
  // printf("res->noise_estimate_dbm = %f \n", res->noise_estimate_dbm);
  // printf("res->cfo = %f \n", res->cfo);
  // printf("res->rsrp = %f\n", res->rsrp);
  // printf("res->rsrp_dbm = %f\n",res->rsrp_dbm);
  // printf("res->rsrp_neigh = %f\n", res->rsrp_neigh);
  // printf(" res->rsrq = %f\n",  res->rsrq);
  // printf(" res->rsrq_db = %f\n", res->rsrq_db);
  // printf("  res->snr_db = %f\n",   res->snr_db);
  // printf(" res->rssi_dbm = %f\n", res->rssi_dbm);
  // printf(" res->sync_error = %f\n", res->sync_error);
  
  // // 将除ce外的数据写入文件
  // // FILE *f;
  // // f = fopen("/home/hadoop/Bjz_workplace/srsLTE-vscode/data/channel_res_data1.csv","a");
  // printf("正在写入文件channel_res_data1.csv...\n");
  // // fprintf(f, "%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f\n", res->noise_estimate, res->noise_estimate_dbm, res->cfo, res->rsrp, res->rsrp_dbm, res->rsrp_neigh, res->rsrq, res->rsrq_db, res->snr_db, res->rssi_dbm, res->sync_error);
  // printf("%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f\n", res->noise_estimate, res->noise_estimate_dbm, res->cfo, res->rsrp, res->rsrp_dbm, res->rsrp_neigh, res->rsrq, res->rsrq_db, res->snr_db, res->rssi_dbm, res->sync_error);
  // printf("文件channel_res_data1.csv完成本轮写入.\n");
  // // fclose(f);

  // 将除ce外的数据写入文件,group:11
  // if (!F3){
  //   // F3=fopen("/home/hadoop/Bjz_workplace/srsLTE-vscode/data/rssi_dbm_data.bin","w+");
  //   F3=fopen("/home/hadoop/Bjz_workplace/srsLTE-vscode/data/rssi_dbm_data.txt","w+");
  // }
  ...
  // // printf("正在写入文件f3...");
  // /* code */
  // fwrite(&(res->noise_estimate), sizeof(float), 1, F3);
  // fwrite(&(res->noise_estimate_dbm), sizeof(float), 1, F3);
  // fwrite(&(res->cfo), sizeof(float), 1, F3);
  // fwrite(&(res->rsrp), sizeof(float), 1, F3);
  // fwrite(&(res->rsrp_dbm), sizeof(float), 1, F3);
  // fwrite(&(res->rsrp_neigh), sizeof(float), 1, F3);
  // fwrite(&(res->rsrq), sizeof(float), 1, F3);
  // fwrite(&(res->rsrq_db), sizeof(float), 1, F3);
  // fwrite(&(res->snr_db), sizeof(float), 1, F3);
  // fwrite(&(res->rssi_dbm), sizeof(float), 1, F3);
  // fwrite(&(res->sync_error), sizeof(float), 1, F3);

  if (!F2){
    F2=fopen("/home/hadoop/Bjz_workplace/srsLTE-github/srsLTE-vscode/data/ce_data1.txt","w+");
  }

  // if (!F3){
  //   F3=fopen("/home/hadoop/Bjz_workplace/srsLTE-vscode/data/sf_data1.txt","w+");
  // }

  if(SRSLTE_NRE * q->cell.nof_prb == 600)
  {
    /* code */
    // printf("正在写入文件f2...");
    for(uint32_t i=0; i<1; ++i){ // nof_ports = 1
      for(uint32_t j=0; j<1; ++j){ // nof_ant = 1
        // fwrite(res->ce[i][j],8,res->nof_re/256,F2);
        //for(uint32_t k=0; k<SRSLTE_NRE * q->cell.nof_prb; k++){ // resource block's first lie
          // printf("%f+%fi\n",crealf(res->ce[i][j][k]), cimagf(res->ce[i][j][k]));
        fwrite(res->ce[i][j],8,SRSLTE_NRE * q->cell.nof_prb,F2);
        //}
        // printf("SRSLTE_NRE * q->cell.nof_prb=%d",SRSLTE_NRE * q->cell.nof_prb);
        // printf("end\n");
        // for (int k = 0; k < 600; k++)
        // {
        //   /* code */
    ...
    // {
    //   /* code */
    //   fprintf(F3, "%6f+%6fi\n",crealf(input[0][k]), cimagf(input[0][k]));
    // }

    // fprintf(F3, "\n");
    // fwrite(input[0], 8, 8400, F3);
